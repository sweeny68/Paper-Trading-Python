...
def master_login():
    # Insert the default login as the first row after creating the table
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute('''
    INSERT INTO staff (dob, first_name, surname, email, phone, username, password)
    VALUES ('2000-01-01', 'Admin', 'User', 'admin@example.com', '0000000000', 'admin', '123');
    ''')
    conn.commit()
    conn.close()
